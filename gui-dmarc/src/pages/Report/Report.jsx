...
import AuthResultsChart, { DMARCResultsChart, PolicyEvaluatedChart } from '@src/components/PieCharts'
import  ChoroplethMap  from '../../components/ChoroplethMap'

function DataCard({ title, children }) {
  return (
    <div className="flex flex-col flex-grow w-fit bg-gray-700 text-white py-4 px-5 items-center justify-center rounded-lg shadow-lg ">
    ...
    <div className="flex flex-col mx-auto w-screen px-20">
      ...
      <div className="flex flex-row gap-2 justify-center w-full flex-wrap">
        ...
        <DataCard title="Date range:">
          <p>
            Begin: {new Date(report.report_metadata.date_range.begin).toLocaleDateString()}
          </p>
          <p>
            End: {new Date(report.report_metadata.date_range.end).toLocaleDateString()}
          </p>
        </DataCard>
        ...
        {report.policy_published && (
          <div className="mt-1 overflow-x-auto">
            <table className="min-w-full bg-gray-700 border">
              ...
              <tbody>
                <tr>
                  <td className="border px-4 py-1">{report.policy_published.domain}</td>
                  <td className="border px-4 py-1">{report.policy_published.adkim}</td>
                  <td className="border px-4 py-1">{report.policy_published.aspf}</td>
                  <td className="border px-4 py-1">{report.policy_published.p}</td>
                  <td className="border px-4 py-1">{report.policy_published.sp}</td>
                  <td className="border px-4 py-1">{report.policy_published.pct}%</td>
                  <td className="border px-4 py-1">{report.policy_published.fo}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div className="flex justify-between w-full py-1 rounded-xl">
        <a href="#policy" className="btn btn-md bg-gray-600 text-gray-100">
          Policy Evaluated
        </a>
        <a href="#auth" className="btn btn-md bg-gray-600 text-gray-100">
          Auth results
        </a>
      </div>
      <div className="flex flex-col w-full">
        ...
        <h2 className="text-2xl font-semibold my-4 ">Geographical Heatmap by IP Addresses</h2>
